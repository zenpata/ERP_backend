# UX Flow — R1-04 HR: การลา (ประเภท, คำขอ, อนุมัติ/ปฏิเสธ)

เอกสารนี้แมป **การทำงานของผู้ใช้** ไปยัง endpoint ใน `Documents/SD_Flow/HR/leaves.md` และข้อกำหนด Feature 1.4 ใน Release 1 (รวมถึง paid/unpaid leave ที่ส่งผล payroll — อ้างอิงใน Notes)

**แหล่งอ้างอิงที่ผูกกับเอกสารนี้**

- Business requirement (BR): `Documents/Requirements/Release_1.md` (Feature 1.4 HR — Leave Management)
- Traceability: `Documents/Requirements/Release_1_traceability_mermaid.md` (Feature 1.4)
- Sequence / SD_Flow: `Documents/SD_Flow/HR/leaves.md`
- Related screens / mockups: `Documents/UI_Flow_mockup/Page/R1-04_HR_Leave_Management/LeaveList.md`

---

## E2E Scenario Flow

> ภาพรวมการลาครอบคลุมตั้งแต่โหลดประเภทลาและยอดคงเหลือ, ยื่นคำขอ, approve/reject, จัดการ leave types, balances และ approval configs โดยยึดกฎว่า balance ถูกหักเมื่อ approve และ unpaid leave ที่อนุมัติแล้วมีผลต่อ payroll
...
flowchart TD
    ...
    START(["▶ เข้าเมนู `/hr/leaves`"]):::terminal
    SCR1["🖥 Leave Workspace
─────────────────────
📋 Requests, Types, Balances, Approval Configs
📋 เห็นสิทธิ์ตาม role
─────────────────────
🔘 [ยื่นลา] 🔘 [อนุมัติ] 🔘 [จัดการ master]"]:::screen
    U1(["👤 เลือก flow
ของพนักงานหรือ HR/approver"]):::user
    D1{{"กำลังทำงานส่วนไหน?"}}:::decision

    START --> SCR1 --> U1 --> D1

    SCR2A["🖥 Leave Request Flow
─────────────────────
📝 leaveTypeId, startDate, endDate, days, reason
📎 attachment (ถ้าจำเป็น)
─────────────────────
🔘 [Submit] 🔘 [Cancel]"]:::screen
    SCR2B["🖥 Approval Flow
─────────────────────
📋 pending requests + balance context
─────────────────────
🔘 [Approve] 🔘 [Reject]"]:::screen
    SCR2C["🖥 HR Master Flow
─────────────────────
📋 leave types, balances, approval configs
─────────────────────
🔘 [Create] 🔘 [Update] 🔘 [Delete]"]:::screen

    D1 -- "ขอลา" --> SCR2A
    D1 -- "อนุมัติ/ปฏิเสธ" --> SCR2B
    D1 -- "จัดการ master" --> SCR2C

    U2(["👤 กรอก / ตรวจ / บันทึกข้อมูล"]):::user
    D2{{"ผ่าน validation
และ business rule หรือไม่?"}}:::decision
    OUT1["✅ คำขอลาถูกสร้างหรืออัปเดตสถานะสำเร็จ"]:::success
    OUT2["✅ Leave types / balances / configs พร้อมใช้งาน"]:::success
    ERR1["⚠ Quota / Config Error
balance ไม่พอ หรือ approval config ไม่พร้อม"]:::error
    ERR2["⚠ Validation / Permission Error
ข้อมูลไม่ครบ หรือไม่มีสิทธิ์"]:::error

    SCR2A --> U2
    SCR2B --> U2
    SCR2C --> U2
    U2 --> D2
    D2 -- "request / approve / reject สำเร็จ" --> OUT1
    D2 -- "master data สำเร็จ" --> OUT2
    D2 -- "quota/config fail" --> ERR1
    D2 -- "validation/permission fail" --> ERR2
    ERR1 --> SCR1
    ERR2 --> SCR1

    ENDNODE(["⏹ End / กลับหน้า Leave Workspace"]):::terminal
    OUT1 --> ENDNODE
    OUT2 --> ENDNODE
```
...
|----------|---------|---------|
| ✅ พนักงานเริ่มยื่นลาได้ถูกประเภท | เปิดหน้า → โหลด leave types และ balance ของตนเอง | เห็นประเภทลาและยอดคงเหลือก่อนยื่น |
| ✅ สร้างคำขอลา | กรอกช่วงวัน/เหตุผล/เอกสาร → submit | ได้ leave request สถานะ `pending` |
| ✅ อนุมัติคำขอ | approver เปิดคำขอ → ตรวจ balance → approve | สถานะเป็น `approved` และระบบหัก balance |
| ✅ ปฏิเสธคำขอ | approver เปิดคำขอ → ระบุเหตุผล → reject | สถานะเป็น `rejected` โดยไม่เปลี่ยน balance |
| ✅ HR จัดการประเภทลาและ quota | ใช้ sections ของ leave types / balances / bulk allocate | master และ quota พร้อมใช้งานในปีนั้น |
| ✅ HR ตั้งค่าสายอนุมัติ | เปิด approval configs → create/update/delete | แผนกมี approver chain ที่พร้อม route คำขอ |
| ⚠ อนุมัติไม่ได้เพราะ quota หรือ config ไม่พร้อม | approver กด approve → balance ไม่พอหรือ config ไม่ครบ | ระบบ block พร้อมข้อความให้ HR แก้ไขก่อน |
| ⚠ ข้อมูลไม่ผ่าน validation หรือไม่มีสิทธิ์ | submit action → field ไม่ครบ / role ไม่ถูกต้อง | แสดง error และให้ retry |
